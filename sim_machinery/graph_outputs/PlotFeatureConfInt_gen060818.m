function  [hl, hp] = PlotFeatureConfInt_gen060818(R,permMod,fighan)
...
for i = 1:N
    for j = 1:M
        ...
        %         end
        [hl, hp] = boundedline(F(:,lr),Y(:,lr),B(:,lr),'cmap',R.plot.cmap,'alpha','transparency',0.45);
        ...
        hold on
        for L = lr
            plot(R.data.feat_xscale,squeeze(R.data.feat_emp(C,i,j,L,:)),'color',[0 0 0]); hold on
        end
        if i == j
            ylim([0 4])
            ylabel('Power')
