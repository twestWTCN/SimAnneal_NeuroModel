   ...
   parfor jj = 1:rep % Replicates for each temperature
        ...
        if ii>1
            for src = 1:length(m.m)
                ...
                T_skew = pskewsave{ii-1}.int{src}.T;
                for Q = 1:length(T_old)
                    T(Q) = pearsrnd(T_old(Q),stdev,T_skew(Q),3,1,1); % Kurtosis set to 3 (normal)
                end
                ...
                G_skew = pskewsave{ii-1}.int{src}.G;
                for Q = 1:length(G_old)
                    G(Q) = pearsrnd(G_old(Q),stdev,G_skew(Q),3,1,1); % Kurtosis set to 3 (normal)
                end
                ...
                % Input Gains (noise)
                C_old =pold.C;
                C_skew = pskewsave{ii-1}.C;
                for Q = 1:length(C_old)
                    C(Q,1) = pearsrnd(C_old(Q),stdev,C_skew(Q),3,1,1); % Kurtosis set to 3 (normal)
                end
                ...
                for Ai = 1:2
                    ...
                    A_skew = pskewsave{ii-1}.A{Ai*2};
                    for Q = 1:length(A_old)
                        A(Q) = pearsrnd(A_old(Q),stdev,A_skew(Q),3,1,1); % Kurtosis set to 3 (normal)
                    end
                ...
                % Lead field (obs gain)
                LF_old = p.obs.LF;
                LF_skew = pskewsave{ii-1}.obs.LF;
                for Q = 1:length(LF_old)
                    LF(Q) = pearsrnd(LF_old(Q),stdev,LF_skew(Q),3,1,1); % Kurtosis set to 3 (normal)
                end                
                ...
                mix_skew = pskewsave{ii-1}.obs.mixing;
                for Q = 1:length(mix_old)
                    mix(Q) = pearsrnd(mix_old(Q),stdev,mix_skew(Q),3,1,1); % Kurtosis set to 3 (normal)
                end                
...
%         pd = tbr2(ii)-tbr2(ii-1); %
        ...
        [pPrecSave{ii},pSkewSave{ii}] = skewResamp(tbr2,psave,ii)
...
%         end
    ...
    if ii<2
        ...
        csdplotter_220517({R.data.meancsd_data},{csd_rep{Ilist(1:10)}},R.frqz,R,I)
    end
    drawnow; shg
