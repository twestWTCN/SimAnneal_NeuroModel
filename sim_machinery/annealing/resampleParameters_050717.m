function pnew = resampleParameters_050717(R,p,PSS,PPS,stdev,MN)
...
for i = 1:length(plist)
    for src = 1:length(MN)
        pold = p;
        if strmatch(plist{i},'.A')  % Weird case for connection strengths where we loop through
            for Ai = 1:2
                X_old = eval(['pold' plist{i} '{Ai*2}']);
                X_skew = eval(['PSS' plist{i} '{Ai*2}']);
                X_prec = eval(['PSS' plist{i} '{Ai*2}']);
