...
for ii = 1:searchN
    stdev = (R.SimAn.jitter)*((0.95*Tm(ii)^2)+0.01); % Set maximum width of distribution from which to mutate
    if ii == 1
        rep = 1;
    else
        rep = repset;
...
%         end
    ...
    if ii<2
        ...
        fx({R.data.feat_emp},{feat_sim_rep{I}},R.data.feat_xscale,R,I)
    end
    drawnow; shg
    
    figure(2)
    clf
    optProgPlot(Tm(1:ii),tbr2,p)
