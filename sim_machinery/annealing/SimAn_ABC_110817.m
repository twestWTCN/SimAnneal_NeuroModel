...
%         R.plot.outFeatFx({},{feat_sim},R.data.feat_xscale,R,1)
    ...
    if size(parBank,2)>R.SimAn.minRank
        if ii == 1
            L = 0.8*rep; % while bank is small
        else
            L = 1*rep;
        end
       
        eps = prctile(parBank(end,end-round(L):end),65); % percentile eps
        eps_tmp = (-3.*Tm(ii))+2; % temperature based epsilon (arbitrary function)
        
        if eps>-0.15 && eps<eps_tmp % steps the annealing into gear once fit is reasonable and if prctile is less than that expected for the given
                                    % temperature
            eps = eps_tmp;
            disp(['Using Annealing EPS'])
        end
        ...
        % copula formation
        if R.SimAn.minRank - size(parOptBank,2) > R.SimAn.minRank/2 && itry >1
            aN = R.SimAn.minRank-size(parOptBank,2);
            mu = mean(parOptBank,2);
            sig = cov(parOptBank');
...
%             end
            itry = 0; 
            disp(['Can`t use temp or % epsilon, using whileloop EPS instead: ' num2str(eps)])
        else
        ...
        if size(parOptBank,2)> R.SimAn.minRank-1 % Ensure rank is larger than parameters
            ...
            iflag = 0; % flag to draw from distribution (should only go up once- following first copula)
...
%% Now draw parameter sets for the next set of replicates    
    ...
    clf
    fx = R.plot.outFeatFx;
    fx({R.data.feat_emp},{feat_sim_rep{Ilist(1:12)}},R.data.feat_xscale,R,1)
    drawnow; shg
    %%%     %%%     %%%     %%%     %%%     %%%     %%%     %%%    
    ...
    if istrue(R.plot.save)
        ...
        disp({['Current R2: ' num2str(tbr2(Ilist(1)))];[' Temperature ' num2str(Tm(ii)) ' K']; R.out.tag; ['Eps ' num2str(eps)]})
    end
