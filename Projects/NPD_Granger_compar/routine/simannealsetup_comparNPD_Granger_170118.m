function R = simannealsetup_comparNPD_Granger_170118()
% 
R.projectn = 'NPD_Granger_compar';
R.out.tag = 'NPD_Grange';
...
%% OPTIMISATION
R.SimAn.pOptList = {'.int{src}.T','.int{src}.G','.C','.obs.LF'}; %,,'.A','.D','.obs.LF'}; ,'.int{src}.S','.S' %,'.C','.obs.LF'}; % ,'.obs.mixing','.C','.D',
R.SimAn.pOptBound = [-12 12];
R.SimAn.pOptRange = R.SimAn.pOptBound(1):.1:R.SimAn.pOptBound(2);
R.SimAn.searchN = 100;
R.SimAn.Tm = 1.5; % Initial temperature
