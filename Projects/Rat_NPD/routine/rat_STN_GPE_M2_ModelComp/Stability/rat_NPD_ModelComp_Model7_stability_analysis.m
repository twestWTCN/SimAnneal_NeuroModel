...
% [xsims dum wflag J Es] = R.IntP.intFx(R,m.x,uc,p,m);
% 
...
NI = 32; NJ = 32;
CswitchI = linspace(-2,2,NI); % Modulating the GPe -| STN (3,2)
CswitchJ = linspace(-2,3,NJ); % Modulating the M2 -> STN
Jsave= zeros(NI,NJ);
STN = zeros(NI,NJ);
GPe = zeros(NI,NJ);
...
for j = 1:NJ
    parfor i = 1:NI
        [j i]
        pnew = p;
        pnew.A{1}(2,3) = CswitchJ(j);
        pnew.A{2}(3,2) = CswitchI(i);
        ...
        [xsims dum wflag J Es] = R.IntP.intFx(R,m.x,u,pnew,m);
        Jsave(i,j) = mean(Es{1});
%         eJ = eig(J{1});
%         Jsave(i,j) =eJ(1);
%         JMS = zeros(4,1);
...
%         Jmodsave(:,i,j) = JMS;
        if wflag == 0
            try
                ...
                STN(i,j) = max(squeeze(feat_sim(1,3,3,1,beta_ind)));
...
colormap(cmap)
subplot(2,1,1)
colGridCon(CswitchI,CswitchJ,(squeeze(real(squeeze(Jmodsave(4,:,:))))),-1.5:0.5:0.5)
xlabel('M2 \rightarrow  STN'); ylabel('GPe \rightarrow STN')
...
subplot(2,1,2)
colGridCon(CswitchI,CswitchJ,(squeeze(imag(squeeze(Jmodsave(4,:,:))))),0:0.25:1)
xlabel('M2 \rightarrow  STN'); ylabel('GPe \rightarrow STN')
