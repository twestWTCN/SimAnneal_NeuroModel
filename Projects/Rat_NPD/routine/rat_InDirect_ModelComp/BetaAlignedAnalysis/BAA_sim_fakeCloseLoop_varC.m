function [R] = BAA_sim_fakeCloseLoop_varC(R,MP,simtime,fresh)
...
R.IntP.intFx = @spm_fx_compile_120319_varC;
m = MP.m;
P = MP.p;
% connection list
...
%             pulseKern = sin(pulse_Phi+(phaseShift(p))); % 
            pU(pulseInds) = repmat(-1,1,pulseWid);
        end
        uc_ip{2} =  uc_ip{1};
        uc_ip{2}{1}(:,end+1) = pU;
%         uc_ip{2}{1}(:,1) = uc_ip{2}{1}(:,1) + pU'; % Give it a cortical pulse
        ...
        figure(100)
        a(1) = subplot(2,1,1)
        plot(R.IntP.tvec_obs,xsim_ip{1}{1}(1,1:end));
        hold on
        plot(R.IntP.tvec_obs,xsim_ip{2}{1}(1,1:end));
        
        a(2) = subplot(2,1,2)
        plot(R.IntP.tvec_obs,xsim_ip{1}{1}(4,1:end)+5e-6);
        hold on
        plot(R.IntP.tvec_obs,xsim_ip{2}{1}(4,1:end)+5e-6);
        linkaxes(a,'x')
        xlim([7 8])
