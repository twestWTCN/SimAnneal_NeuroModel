...
% 
% subplot(1,3,3)
% p(1) = plot(conStren.*100,max(stn_maxpow_nmz));
% p(1).LineStyle = '-.';
% hold on
% sc(1) = scatter(conStren(NcS_sel).*100,max(stn_maxpow_nmz(:,NcS_sel)),50,condcmap(6+NcS_sel,:),'filled');
% sc(1).Marker = 'diamond';
% p(2) = plot(conStren.*100,min(stn_maxpow_nmz));
% p(2).LineStyle = '--';
% hold on
% sc(2) = scatter(conStren(NcS_sel).*100,min(stn_maxpow_nmz(:,NcS_sel)),50,condcmap(6+NcS_sel,:),'filled');
% sc(2).Marker = 'square';
% 
% p(3) = plot(conStren.*100,range(stn_maxpow_nmz));
% p(3).LineStyle = '-';
% hold on
% sc(3) = scatter(conStren(NcS_sel).*100,range(stn_maxpow_nmz(:,NcS_sel)),50,condcmap(6+NcS_sel,:),'filled');
% sc(3).Marker = 'o';
% 
